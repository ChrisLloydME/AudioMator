...
| Raw metadata editor | App-hosted tests cover draft row construction, multi-target edits, raw property-map apply, partial-failure retry scope, deletion, text utilities, and reload behavior. | `MetadataEditorStoreTests.swift`, `MetadataEditorDraftRowsTests.swift`, `InspectorAndMetadataEditorWorkflowTests.swift` |
...
| Online metadata providers | Fast and app-hosted tests cover deterministic MusicBrainz/iTunes request/query/link helpers, selection summaries, comparison rows, captured plan snapshots, file fingerprint rejection, result models, extreme numeric inputs, explicit state mapping, and restarted-search cancellation races. | `MusicBrainz*Tests.swift`, `iTunesMetadataComparisonBuilderTests.swift`, `ProviderSearchRestartTests.swift`, `OnlineMetadataPlanSnapshotTests.swift`, `AudioMatorCoreLogicTests.swift` |
| LRCLIB lyrics | Fast and app-hosted tests cover request building, decoding, overflow-safe ranking, store no-result/cancel/queue and restarted-search behavior, and the synced-lyrics raw property-map write path. | `LRCLIBLyricsTests.swift`, `AudioMatorCoreLogicTests.swift` |
...
| Concurrent or cancelled file mutations can overlap or execute late | Covered by deterministic queued-reservation tests for write, erase, renumber, normalized URL aliases, cancelled waiters, and follow-up reservation reuse. |
