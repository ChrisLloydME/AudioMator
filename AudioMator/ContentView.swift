    ...
    var body: some View {
        ...
        .sheet(isPresented: $isTrackRenumberPresented) {
            VStack(alignment: .leading, spacing: 14) {
                ...
                if trackRenumberResult.totalTargets > 0 || trackRenumberResult.failed > 0 || trackRenumberResult.skippedUnsupported > 0 {
                    GroupBox {
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            if !trackRenumberResult.failures.isEmpty {
                                ...
                                ScrollView {
                                    VStack(alignment: .leading, spacing: 6) {
                                        ForEach(trackRenumberResult.failures) { f in
                ...
                HStack {
                    ...
                    Button("Close") {
                        isTrackRenumberPresented = false
                    }
                    .keyboardShortcut(.cancelAction)
                    .disabled(isTrackRenumberRunning)

                    Button("Apply") {
                        // Parse start number safely
                        ...
                        // orderedIDs comes from the middle-list ordering
